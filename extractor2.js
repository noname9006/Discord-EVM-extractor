...
async function extractMessages(channel, startDate, endDate, filePath) {
    ...
    while (true) {
        ...
        // Release references to messages to avoid memory leaks
        fetchedMessages.clear();

        // Add a limit to prevent infinite loops in case of unexpected issues
        if (totalMessagesFetched >= 10000) {
            console.log('Reached limit of 10000 messages fetched, stopping extraction.');
            break;
        }
    }
